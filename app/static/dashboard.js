...
function updatePatrolPanel(state) {
  ...
  const violationPanel = document.getElementById("patrolViolations");
  const violationCount = document.getElementById("violationCount");
  const violations = state.violations || [];
  violationCount.textContent = violations.length;
  violationPanel.style.display = violations.length > 0 ? "block" : "none";
  if (violations.length > 0) {
    violationList.innerHTML = violations.map(v => {
      const pos = (v.map_x != null && v.map_y != null)
        ? `(${v.map_x.toFixed(1)}, ${v.map_y.toFixed(1)})` : "—";
      const photo = v.photo
        ? `<div class="violation-item-photo"><a href="${v.photo}" target="_blank"><img src="${v.photo}" alt="violation"></a></div>`
        : "";
      return `<div class="violation-item">
        <div class="violation-item-rule">&#9888; ${escHtml(v.rule_text)}</div>
        <div class="violation-item-meta">
          <span>${v.timestamp || "—"}</span>
          <span>${pos}</span>
          <span>${(v.confidence * 100).toFixed(0)}%</span>
          <span>${v.detected_objects ? v.detected_objects.join(", ") : ""}</span>
          ${v.voice_alert_played ? '<span>已语音告警</span>' : ''}
        </div>
        ${photo}
      </div>`;
    }).join("");
...
function renderPatrolReport(report) {
  ...
  if (violations.length > 0) {
    html += `<div class="report-persons" style="margin-top:10px"><strong>违规记录 (${violations.length}):</strong>
      <table class="report-table">
        <tr><th>#</th><th>规则</th><th>检测物品</th><th>时间</th><th>位置</th><th>照片</th></tr>`;
    violations.forEach((v, i) => {
      const pos = (v.map_x != null && v.map_y != null)
        ? `(${v.map_x.toFixed(2)}, ${v.map_y.toFixed(2)})` : "—";
      const photo = v.photo
        ? `<a href="${v.photo}" target="_blank"><img src="${v.photo}" class="report-thumb"></a>`
        : "—";
      html += `<tr>
        <td>${i + 1}</td>
        <td>${escHtml(v.rule_text || "")}</td>
        <td>${(v.detected_objects || []).join(", ")}</td>
        <td>${v.timestamp || "—"}</td>
        <td>${pos}</td>
        <td>${photo}</td>
      </tr>`;
...
async function refreshRules() {
  try {
    const data = await fetchJSON("/api/patrol/rules");
    if (data.ok) {
      appState.patrolRules = data.rules || [];
      updateRuleList();
    }
  } catch (_err) {}
}
...
function updateRuleList() {
  ...
  if (!appState.patrolRules.length) {
    container.innerHTML = '<span class="hint">添加文字规则，系统自动匹配检测类别</span>';
    return;
  }
  container.innerHTML = appState.patrolRules.map(rule => {
    const cls = rule.enabled ? "" : " disabled";
    const classes = (rule.target_classes || []).join(", ");
    return `<div class="rule-item${cls}" data-rule-id="${rule.id}">
      <button class="rule-item-toggle" onclick="handleToggleRule('${rule.id}')">
        ${rule.enabled ? "启用" : "禁用"}
      </button>
      <span class="rule-item-text">${escHtml(rule.text)}</span>
      <span class="rule-item-classes">[${classes}]</span>
      <button class="rule-item-delete" onclick="handleDeleteRule('${rule.id}')">&times;</button>
    </div>`;
  }).join("");
}
...
async function handleAddRule() {
  ...
  if (!text) return;
  const res = await fetchJSON("/api/patrol/rules", {
    method: "POST",
    headers: {"Content-Type": "application/json"},
    body: JSON.stringify({text}),
  });
  if (res.ok) {
    ...
    await refreshRules();
    document.getElementById("commandResult").textContent =
      `规则已添加: "${res.rule.text}" → [${res.rule.target_classes.join(", ")}]`;
  }
}

async function handleDeleteRule(ruleId) {
  await fetchJSON(`/api/patrol/rules/${ruleId}`, {method: "DELETE"});
  await refreshRules();
}

async function handleToggleRule(ruleId) {
  await fetchJSON(`/api/patrol/rules/${ruleId}/toggle`, {method: "POST"});
  await refreshRules();
}
...
function drawViolationMarkers() {
  ...
  if (!ps || !ps.violations) return;

  ps.violations.forEach(v => {
    if (v.map_x == null || v.map_y == null) return;
    const p = worldToScreen(v.map_x, v.map_y);
    const size = 7;
    ctx.beginPath();
